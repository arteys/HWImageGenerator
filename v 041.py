...
def Draw_Polygons(polygons, color, object_flag, x_lim, y_lim, filename, folder):
    ...
    blurImage.save(folder + str(filename) + str(color) + ".png","PNG")

def Voronoi_grid(side_x, side_y, variation):
    ...
    cn, rn = side_x, side_y # number of columns/rows
...
def Cell_generation(side_x, side_y, variation):
    vor, x_points, y_points = Voronoi_grid(side_x, side_y, variation)
...
for number in range(1,15):
    folder = 'D:/From G-drive/Python/Cells/'
    filepath = folder + str(number) + ".csv"
    
    rng = np.random.default_rng()

    side_x = rng.integers(15,25)
    side_y = rng.integers(15,25)

    list_of_polygons, list_of_circles, pnts_in_poly, list_of_area, list_of_nucleus_r = Cell_generation(side_x, side_y, 0.1)
 
    Draw_Polygons(list_of_polygons, "green", "poly", x_lim_const, y_lim_const,  str(number), folder)
    Draw_Polygons(list_of_circles, "blue", "poly", x_lim_const, y_lim_const,  str(number), folder)
    Draw_Polygons(pnts_in_poly, "red", "dots", x_lim_const, y_lim_const, str(number), folder)


    array_area = np.array(list_of_area)
    array_radii = np.array(list_of_nucleus_r)

    d = {'Area':[array_area], 'Radii': [array_radii]}
