...
for region in vor.regions:
    ...
    if (any(i<0 for i in region) == False) and (len(region) > 0):
        ...
        if np.max(abs(polygon_vertices)) < 1.2*abs(max(x_points)):
            ...
            nucleus_r = distance_from_center_to_exterior*70
...
# p1 = figure(width=1024, height=1024)
# p2 = figure(width=1024, height=1024)

# p1.background_fill_color = "black"
# p1.xgrid.grid_line_color = None
# p1.ygrid.grid_line_color = None
# p1.patches(list_polygons_xs, list_polygons_ys, color="green")

# p2.background_fill_color = "black"
# p2.xgrid.grid_line_color = None
# p2.ygrid.grid_line_color = None
# p2.circle(list_centres_xs, list_centres_ys, color="blue", size=list_of_nucleus_r)

# # show(row(p1,p2))
# export_png(p1, filename="plot.png")
# export_png(p2, filename="plot_2.png")
